# Fresh Install

[![Volver al Inicio](https://img.shields.io/badge/-Volver_al_Inicio-6e5494?style=for-the-badge&logo=home-assistant&logoColor=white&labelColor=1a1a1a)](README.md)

## Pacman Tweaks

```zsh
sudo vim /etc/pacman.conf
```

- Descomentar:
  - #`Color`
  - #`VerbosePkgLists`

- Modificar:
  - Agregar `ILoveCandy` debajo de `Color`
  - Cambiar `#ParallelDownloads = 5` por `ParallelDownloads = 10`

- Guardar y salir:
  - `Shift + wq`

## Instalar Yay (Bin)

```zsh
sudo pacman -S --needed git base-devel && git clone https://aur.archlinux.org/yay-bin.git && cd yay-bin && makepkg -si
```

## Instalar Paquetes

Primero instalamos y configuramos `rustup` para asegurar que el ecosistema de Rust se instale por el toolchain stable y no por dependencias indirectas de `pacman`:

```zsh
sudo pacman -S --needed rustup && rustup default stable
```

Luego instalamos el resto de los paquetes:

```zsh
sudo pacman -S --needed gnome-keyring gnome-shell-extension-appindicator noto-fonts-emoji firefox libreoffice-fresh libreoffice-fresh-es kitty nushell zsh nodejs gufw ttf-jetbrains-mono-nerd ttf-ubuntu-font-family yazi ffmpeg 7zip jq poppler fd ripgrep fzf zoxide resvg imagemagick libayatana-appindicator keepassxc signal-desktop proton-vpn-gtk-app lazygit less reflector pacman-contrib starship xclip wl-clipboard xdg-user-dirs fastfetch helix eza bat unclutter cava asciiquarium pnpm yt-dlp ty tombi timeshift sudo-rs rust-analyzer rsync rclone postgresql obsidian marksman markdownlint-cli btop gemini-cli flatpak cowsay uv ruff podman vscode-css-languageserver vscode-json-languageserver code veracrypt dysk && yay -S cryptomator windsurf antigravity antigravity-cli phoenix-arch carapace-bin unimatrix-git
```

- Elimina `gnome-keyring` si ya tienes un agente de autenticación
- Elimina `gnome-shell-extension-appindicator` si no usas GNOME
- Elimina `xclip` si estás en un DE/WM con Wayland
- Elimina `wl-clipboard` si estás en un DE/WM con X11
- Elige tu IDE favorito: `windsurf`,`visual-studio-code-bin` o `antigravity`
- Elimina `phoenix-arch` si prefieres un Firefox limpio o prefieres usar [Betterfox](firefox+uBO/user.js)
